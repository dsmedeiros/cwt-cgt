...
### 36.4a Named Benchmark D continuous Lindblad specialization

The isolated harness at `cwt-sim/experiments/benchmark_d_lindblad_response_proof/` instantiates the separately scoped continuous-time contraction corollary for the same named Benchmark D `D0` kernel and `mean_position` readout, but not for the Phase 11 Euler trajectory. It fixes $b\in[0.01,0.05]$, $d\in[0.205,0.245]$, center $(0.03,0.225)$, edge-jump scale $1/5$, depolarization rate $1/25$, dephasing $3/10$, and zero coherent/site-potential terms. On diagonal densities the exact core Lindblad RHS is
\[
\dot x=A(b,d)x+c,
\qquad A=\frac15(K^T-I)-\frac1{25}I,
\qquad c=\frac1{125}{\bf1}.
\]
The core RHS and superoperator are checked on the complete diagonal population and traceless-diagonal deviation bases at all four box corners and the center; this is diagonal-invariant-subspace equivalence, not a full-superoperator claim. The jump-plus-dephasing evolution is trace-norm contractive and depolarization gives the common bound $\|U(t,s)\|_1\le e^{-(t-s)/25}$ on traceless Hermitian deviations, hence $M=1$, $\tau=25$, and $\|A^{-1}\|\le25$. The exact stationary branch $\bar x=-A^{-1}c$ has zero rational residual, unit trace, and the analytic component floor $4/69$ throughout the box.

For the centered continuous model-time response $Q=\int_0^T\operatorname{Tr}[O(\rho-\bar\rho)]dt$, exact rational implicit differentiation gives
\[
F_{bd}=\partial_bB_d-\partial_dB_b
=-\frac{28888766872100000000000}{235345963257301712101}
\approx-122.7502119529289.
\]
