...
- **PASS** `G0` — exact_config_control_box_and_no_defaults
- **PASS** `G1` — same_generator_source_stationary_geometry_counting_response
- **PASS** `G2` — unique_uniformly_full_rank_contracting_stationary_branch
- **PASS** `G3` — exact_Drazin_and_branch_derivative_identities
- **PASS** `G4` — actual_branch_projective_and_SLD_geometry
- **PASS** `G5` — tilted_generator_orientation_TP_and_current_derivative
- **PASS** `G6` — exact_response_one_form_and_curvature
- **PASS** `G7` — same_curvature_and_zero_preserving_homogeneous_map_refusal_only
- **PASS** `G8` — authenticated_geometry_counting_oracle_firewalls
- **PASS** `G9` — prediction_lock_and_positive_inference_refusal
- **PASS** `G10` — covariance_closure_units_zero_sets_and_obstructions
- **PASS** `G11` — reverse_count_identity_h_zero_and_same_model_nulls
- **PASS** `G12` — exact_orientation_qanti_factor_two_and_local_curvature_scope
- **PASS** `G13` — immutable_registry_and_claim_semantics

## Claim ceiling

internal exact Benchmark-D D0 shared-generator counting-curvature classifications only; refutes only SAME_CURVATURE and frozen zero-preserving homogeneous Omega-only maps for the declared state geometry; affine, nonlinear, and generator-dependent maps remain open; not universal or full CWT, calibrated physical response, or empirical evidence
