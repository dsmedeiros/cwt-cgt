"""Immutable fail-closed gate and case classifier."""

from __future__ import annotations

from dataclasses import dataclass
from typing import Any, Mapping

from .contract import case_gate_ownership, expected_case_dispositions


@dataclass(frozen=True)
class Gate:
    name: str
    natural_status: str
    status: str
    requirement: str
    observed: Any

    @property
    def natural_passed(self) -> bool:
        return self.natural_status == "pass"

    @property
    def passed(self) -> bool:
        return self.status == "pass"

    def jsonable(self) -> dict[str, Any]:
        return {
            "name": self.name,
            "status": self.status,
            "natural_status": self.natural_status,
            "requirement": self.requirement,
            "observed": self.observed,
        }


def registry_gate_names() -> tuple[str, ...]:
    flattened = [name for _, names in case_gate_ownership() for name in names]
    if len(flattened) != len(set(flattened)):
        raise RuntimeError("every live gate must have exactly one canonical owner")
    return tuple(flattened)


def gate_owner(gate_name: str) -> str:
    owners = [case_id for case_id, names in case_gate_ownership() if gate_name in names]
    if len(owners) != 1:
        raise KeyError(f"gate must have exactly one canonical owner: {gate_name}")
    return owners[0]


def apply_fail_only_overrides(
    natural: Mapping[str, tuple[bool | None, str, Any]],
    overrides: Mapping[str, bool] | None = None,
) -> list[Gate]:
    expected = set(registry_gate_names())
    if set(natural) != expected:
        raise RuntimeError(
            f"live gate registry mismatch: absent={sorted(expected-set(natural))}, "
            f"orphan={sorted(set(natural)-expected)}"
        )
    requested = dict(overrides or {})
    unknown = sorted(set(requested) - expected)
    if unknown:
        raise KeyError(f"unknown gate overrides: {unknown}")
    if any(not isinstance(value, bool) for value in requested.values()):
        raise TypeError("gate override values must be booleans")
    gates = []
    for name in registry_gate_names():
        value = natural[name][0]
        if value not in (True, False, None):
            raise TypeError(f"natural gate status must be True, False, or None: {name}")
        natural_status = "indeterminate" if value is None else ("pass" if value else "fail")
        status = "fail" if requested.get(name) is False else natural_status
        gates.append(
            Gate(
                name=name,
                natural_status=natural_status,
                status=status,
                requirement=natural[name][1],
                observed=natural[name][2],
            )
        )
    return gates


def case_dispositions(gates: list[Gate]) -> dict[str, str]:
    names = tuple(gate.name for gate in gates)
    if names != registry_gate_names() or len(names) != len(set(names)):
        raise RuntimeError("case classification requires the ordered unique canonical gate registry")
    statuses = {gate.name: gate.status for gate in gates}
    expected = expected_case_dispositions()
    result = {}
    for case_id, owned_names in case_gate_ownership():
        failed = [name for name in owned_names if statuses[name] == "fail"]
        indeterminate = [name for name in owned_names if statuses[name] == "indeterminate"]
        if failed:
            result[case_id] = f"FAIL_INTERNAL_ANALYTIC:{','.join(failed)}"
        elif indeterminate:
            result[case_id] = f"INDETERMINATE_INTERNAL_ANALYTIC:{','.join(indeterminate)}"
        else:
            result[case_id] = expected[case_id]
    return result
