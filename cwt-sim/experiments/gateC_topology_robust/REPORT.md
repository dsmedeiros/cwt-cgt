...
**Claim scope:** protocol/noise robustness only, not topology or quantization. The
response was produced by a flux-conditioned memory/readout. Thresholds check only
pure-state coherence/overlap criteria; no mixed-state fallback is implemented.
...
- σ_phase = 0.000 (σ_amp=0.020, σ_tau=0.020)
  ...
  - Pure-state coherence/overlap criteria met: yes
...
- σ_phase = 0.350 (σ_amp=0.020, σ_tau=0.020)
  ...
  - Pure-state coherence/overlap criteria met: no

When either threshold is violated, the point is outside the configured pure-state
criteria; no mixed-state fallback is implemented.
