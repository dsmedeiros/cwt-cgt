...
from cwt.metrics.triage import TRIAGE_WEIGHT_SUM, TRIAGE_WEIGHTS, triage_score
...
def test_triage_weights_sum_to_one() -> None:
    ...
    assert np.isclose(TRIAGE_WEIGHT_SUM, 1.0)
...
def test_triage_weights_mapping_is_read_only() -> None:
    try:
        TRIAGE_WEIGHTS["degree"] = 0.1  # type: ignore[index]
    except TypeError:
        pass
    else:  # pragma: no cover - defensive assertion
        raise AssertionError("TRIAGE_WEIGHTS should be immutable.")
