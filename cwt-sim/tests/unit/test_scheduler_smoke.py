...
def test_run_parameter_loop_respects_tau_knobs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert run_with({"rho": 0.25}) == pytest.approx(1.0)
...
def test_resolve_tau_scale_does_not_warn_when_tau_and_tau_scale_are_close() -> None:
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        resolved = scheduler._resolve_tau_scale({"tau": 1.0, "tau_scale": 1.0 + 1e-13})
