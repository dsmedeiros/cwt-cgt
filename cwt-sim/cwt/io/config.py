...
def _translate_readout_settings(readout: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "kind": str(readout.get("type", readout.get("kind", "stochastic"))),
        "temperature": float(readout.get("T", readout.get("temperature", 1.0))),
        "memory_form": str(readout.get("memory_form", "current_coupled")),
        "params": dict(readout.get("params", {})),
    }


def _translate_noise_settings(noise: Mapping[str, Any]) -> dict[str, float]:
    phase_std = float(noise.get("phase_std", noise.get("theta_std", 0.0)))
    amp_std = float(noise.get("amp_noise", noise.get("prob_std", 0.0)))
    delay_std = float(noise.get("delay_std", 0.0))
    return {
        "theta_std": phase_std,
        "prob_std": amp_std,
        "delay_std": delay_std,
...
def run_config_from_sections(
    *,
    ...
    return RunConfig(
        eta_q=float(dynamics.get("eta_q", 0.3)),
        zeta=float(dynamics.get("zeta", 0.2)),
        omega_scale=float(dynamics.get("omega_scale", 1.0)),
        s_min=float(geometry.get("s_min", 0.6)),
        smooth_window=int(geometry.get("smooth_window", 5)),
        compute_metric=bool(geometry.get("compute_metric", True)),
        compute_curvature=bool(geometry.get("compute_curvature", True)),
        adapt_levels=int(geometry.get("adapt_levels", 2)),
        ci_tol=float(geometry.get("ci_tol", 0.05)),
        alpha=float(geometric_coupling.get("alpha", 0.2)),
        beta=float(geometric_coupling.get("beta", 1.0)),
