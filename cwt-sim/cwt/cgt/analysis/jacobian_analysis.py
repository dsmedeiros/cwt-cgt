...
def _bundle_for_point(
    ...
    bu, bv = control_coupling_vectors(
        benchmark, u=float(u), v=float(v), branch_id=branch_id, reference_state=state, config=config
    )
...
def jacobian_scan_payload(
    ...
    for i, u in enumerate(grid_u):
        for j, v in enumerate(grid_v):
            ...
            if (
                bundle["diagnostics"].spectral_radius > 0.98
                or bundle["diagnostics"].inverse_min_singular_i_minus_m > 15.0
            ):
                warning_tiles.append(
                    {
                        ...
                        "inverse_min_singular_i_minus_m": float(
                            bundle["diagnostics"].inverse_min_singular_i_minus_m
                        ),
    ...
    for i in range(rows - 1):
        for j in range(cols - 1):
            ...
            for ii, jj in idxs:
                ...
                frames.append(
                    _bundle_for_point(
                        benchmark, float(grid_u[ii]), float(grid_v[jj]), branch_id, jac_config, cache
                    )["frame"]
                )
            modal_plaquette_phase[i, j] = mode_wilson_phase(frames)
            mixing_plaquette[i, j] = float(
                np.mean([off_diagonal_mixing_score(a, b) for a, b in zip(frames[:-1], frames[1:])])
            )
    ...
    payload = {
        ...
        "correlations": {
            ...
            "predicted_curvature_vs_state_curvature": _corr(
                predicted_curv_trim.ravel(), curvature_ref.ravel()
            ),
        ...
        "notes": [
            "Phase 6 uses the explicit numerical Jacobian of the branch-local relaxation map,"
            " not the earlier auxiliary modal surrogate.",
...
def jacobian_loop_payload(
    ...
    jac_config = jacobian_config or JacobianConfig()
    cache: dict[tuple[str, float, float, str], dict[str, Any]] = {}
    loop_payload = run_benchmark_loops(
        benchmark_id,
        output_root=output_root,
        config=loop_config,
        protocol_name=protocol_name,
        filename=f"{benchmark_id}_loops.json",
    )
    run_records = []
    for pair in loop_payload["pair_results"]:
        for run in [pair["ccw"], pair["cw"]]:
            ...
            path = build_loop_path(
                ...
                run["side_length"],
                run["orientation"],
                loop_payload["loop_family"]["shape"],
                loop_payload["loop_family"]["steps_per_segment"],
            )
            ...
            center_bundle = _bundle_for_point(
                benchmark,
                float(center[0]),
                float(center[1]),
                benchmark.canonical_branch_id,
                jac_config,
                cache,
            )
            predicted_flux_center = float(center_bundle["curvature_pred"] * run["signed_area"])
            predicted_flux_pathmean = (
                float(np.mean(path_curvatures) * run["signed_area"]) if path_curvatures else 0.0
            )
            jacobian_phase = mode_wilson_phase(frames)
            avg_mixing = (
                float(np.mean([off_diagonal_mixing_score(a, b) for a, b in zip(frames[:-1], frames[1:])]))
                if len(frames) > 1
                else 0.0
            )
            run_records.append(
    ...
    payload = {
        ...
        "summary": {
            ...
            "fit_response_vs_predicted_signed_flux_by_center": _centerwise_fit(
                run_records, "predicted_signed_flux_pathmean", "response"
            ),
            "fit_predicted_signed_flux_vs_state_signed_flux_by_center": _centerwise_fit(
                run_records, "signed_flux", "predicted_signed_flux_pathmean"
            ),
            ...
            "max_inverse_min_singular_i_minus_m_summary": summarize(
                [run["max_inverse_min_singular_i_minus_m"] for run in trusted]
            ),
            "mean_nonnormality_ratio_summary": summarize([run["mean_nonnormality_ratio"] for run in trusted]),
        },
        "notes": [
            "Predicted signed flux uses the path-mean Jacobian-derived curvature multiplied by the"
            " signed loop area.",
            "Jacobian modal phases are retained as a secondary diagnostic but are weaker than the derived"
            " predicted-flux law in the current implementation.",
        ],
    }
    out_dir = output_root / benchmark.slug
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / (filename or f"{benchmark.benchmark_id}_jacobian_loops.json")).write_text(
...
def patchwise_jacobian_explanation(output_root: Path, jacobian_config: JacobianConfig | None = None) -> dict:
    ...
    for protocol in protocol_suite("benchmark_c"):
        ...
        for pair in loops["pair_results"]:
            ...
            for run in [pair["ccw"], pair["cw"]]:
                path = build_loop_path(
                    ...
                    run["side_length"],
                    run["orientation"],
                    loops["loop_family"]["shape"],
                    loops["loop_family"]["steps_per_segment"],
                )
                continuation = continue_path_with_branch_ids(benchmark, path, protocol.config)
                path_curvatures = []
                for (u, v), branch_id in zip(path, continuation["branch_ids"]):
                    path_curvatures.append(
                        _bundle_for_point(benchmark, float(u), float(v), branch_id, jac_config, cache)[
                            "curvature_pred"
                        ]
                    )
        ...
        for center, rec in per_center.items():
            center_accumulator.setdefault(
                center, {"resp_vs_pred": [], "pred_vs_state": [], "resp_vs_state": []}
            )
            ...
            center_accumulator[center]["resp_vs_pred"].append(
                _fit_through_origin(pred_flux, response)["slope"]
            )
            center_accumulator[center]["pred_vs_state"].append(
                _fit_through_origin(state_flux, pred_flux)["slope"]
            )
            center_accumulator[center]["resp_vs_state"].append(
                _fit_through_origin(state_flux, response)["slope"]
            )
        ...
        center_summaries.append(
            {
                ...
                "relative_error": (
                    None
                    if abs(resp_vs_state) <= 1e-12
                    else float(abs(reconstructed - resp_vs_state) / abs(resp_vs_state))
                ),
            }
        )

    observed = np.asarray(
        [item["mean_response_vs_state_flux_slope"] for item in center_summaries], dtype=float
    )
    reconstructed = np.asarray(
        [item["reconstructed_response_vs_state_flux_slope"] for item in center_summaries], dtype=float
    )
    payload = {
        ...
        "notes": [
            "Patchwise signed-loop slopes are decomposed into a response-vs-predicted-flux factor"
            " and a predicted-flux-vs-state-flux factor.",
            "This is the main explanatory result of the explicit Jacobian route in the current"
            " implementation.",
        ],
    }
    out_dir = output_root / benchmark.slug
...
def benchmark_jacobian_payload(
    ...
    scan_payload = jacobian_scan_payload(
        benchmark_id, output_root=output_root, scan_config=scan_config, jacobian_config=jacobian_config
    )
    loop_payload = jacobian_loop_payload(
        benchmark_id, output_root=output_root, config=loop_config, jacobian_config=jacobian_config
    )
    payload = {"benchmark": benchmark_id, "scan": scan_payload, "loops": loop_payload}
    if benchmark_id == "benchmark_c":
        payload["patchwise_explanation"] = patchwise_jacobian_explanation(
            output_root=output_root, jacobian_config=jacobian_config
        )
