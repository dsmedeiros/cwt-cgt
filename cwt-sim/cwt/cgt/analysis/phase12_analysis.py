...
def _family_for_benchmark(benchmark_id: str) -> LoopFamilySpec:
    ...
    if benchmark_id == 'benchmark_d':
        return LoopFamilySpec(shapes=('square', 'circle'), centers=((0.0, 0.225), (0.03, 0.225)), side_lengths=(0.04, 0.06, 0.08), steps_per_segment=18)
    if benchmark_id == 'benchmark_f':
        return LoopFamilySpec(shapes=('square', 'circle'), centers=((0.0, 0.0), (0.7, 0.0), (-0.7, 0.0)), side_lengths=(0.18, 0.30), steps_per_segment=18)
...
def _backend_loop_payload(benchmark_id: str, family: LoopFamilySpec, dephasing_values: tuple[float, ...], cache: DensityCache, step_fn: Callable, backend_name: str) -> dict:
    benchmark = get_benchmark(benchmark_id)
    ...
    gamma0_rows = [rows[0] for rows in pair_series.values() if abs(rows[0]['dephasing'] - gamma0) < 1e-9 and rows[0]['trusted_pair']]
    ...
    for gamma_index, gamma in enumerate(dephasing_values):
        ...
        for shape in family.shapes:
            for center_index, center in enumerate(family.centers):
                ...
                patch_rows = [row for row in trusted if row['shape'] == shape and row['center_index'] == center_index]
...
def _plot_lines(path: Path, x: np.ndarray, ys: list[np.ndarray], labels: list[str], xlabel: str, ylabel: str, title: str) -> None:
...
def _plot_scatter(path: Path, xs: np.ndarray, ys: np.ndarray, xlabel: str, ylabel: str, title: str, fit: dict[str, float | int | None]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
...
def phase12_payload(output_root: Path, phase12_config: Phase12Config | None = None, scan_config: ScanConfig | None = None, open_config: OpenSystemConfig | None = None, lindblad_config: LindbladConfig | None = None) -> dict:
    cfg = phase12_config or Phase12Config()
    scan_cfg = scan_config or ScanConfig(mesh=cfg.scan_mesh)
    open_cfg = open_config or OpenSystemConfig(scan_mesh=cfg.scan_mesh, dephasing_values=cfg.dephasing_values, coherence_switch_floor=cfg.coherence_switch_floor)
    ...
    for benchmark_id in cfg.benchmark_ids:
        ...
        payload = _backend_loop_payload(benchmark_id, family, cfg.dephasing_values, lind_cache, lind_step, 'lindblad')
    ...
    compare_eff = _backend_loop_payload(compare_id, compare_family, cfg.dephasing_values, eff_cache, eff_step, 'effective')
    ...
    lind_scan_levels = [_scan_level(compare_id, scan_cfg, gamma, lind_cache) for gamma in cfg.dephasing_values]
    ...
    for i in range(scan_cfg.mesh):
        for j in range(scan_cfg.mesh):
            if atlas['ambiguous_map'][i][j]:
                continue
            state = states[i][j]
            mean_bures.append(bures_distance_sq(eff_cache.get(state, common_gamma), lind_cache.get(state, common_gamma)))

    lind_slope_seq = np.asarray([level['fit_orientation_gap_vs_aligned_holonomy']['slope'] if level['fit_orientation_gap_vs_aligned_holonomy']['slope'] is not None else np.nan for level in compare_lind['loop_levels']], dtype=float)
    eff_slope_seq = np.asarray([level['fit_orientation_gap_vs_aligned_holonomy']['slope'] if level['fit_orientation_gap_vs_aligned_holonomy']['slope'] is not None else np.nan for level in compare_eff['loop_levels']], dtype=float)
    lind_r2_seq = np.asarray([level['fit_orientation_gap_vs_aligned_holonomy']['r2'] if level['fit_orientation_gap_vs_aligned_holonomy']['r2'] is not None else np.nan for level in compare_lind['loop_levels']], dtype=float)
    eff_r2_seq = np.asarray([level['fit_orientation_gap_vs_aligned_holonomy']['r2'] if level['fit_orientation_gap_vs_aligned_holonomy']['r2'] is not None else np.nan for level in compare_eff['loop_levels']], dtype=float)
    ...
    comparison = {
        ...
        'metric_correlation_at_common_gamma': _corr(np.asarray(lind_scan['bures_metric_trace'], dtype=float), np.asarray(eff_scan['bures_metric_trace'], dtype=float)),
        'curvature_correlation_at_common_gamma': _corr(np.asarray(lind_scan['mixed_curvature'], dtype=float), np.asarray(eff_scan['mixed_curvature'], dtype=float)),
    ...
    payload = {
        'phase': 'phase12_dense_mixed_phase_convention',
        'description': 'Dense dephasing rerun with broader loop families and a benchmark-intrinsic mixed-state phase convention aligned to the pure-state flux reference.',
        ...
        'notes': [
            'The phase convention first unwraps per-loop mixed holonomy phases across γ and then harmonizes the sign against the pure-state flux gap at γ=0.',
            'This does not promote the mixed-state slope to a topology-grade invariant, but it makes raw fitted slopes backend-comparable inside the benchmark scaffold.',
            'Phase 12 targets the positive mixed-state benchmark C; control conclusions from Phase 11 are carried forward unchanged rather than fully rerun on the dense grid in this pass.',
        ],
    }
...
def phase12_report(output_root: Path, payload: dict) -> dict[str, Path]:
    ...
    _plot_lines(slope_path, x, [lind_slope, eff_slope], ['lindblad', 'effective'], 'dephasing γ', 'aligned fit slope', 'Benchmark C aligned mixed-state slope vs dephasing')
    r2_path = plots_dir / 'aligned_r2_vs_dephasing.png'
    _plot_lines(r2_path, x, [lind_r2, eff_r2], ['lindblad', 'effective'], 'dephasing γ', 'fit R²', 'Benchmark C aligned mixed-state fit quality vs dephasing')
    curv_path = plots_dir / 'backend_curvature_vs_dephasing_dense.png'
    _plot_lines(curv_path, x, [lind_curv, eff_curv], ['lindblad', 'effective'], 'dephasing γ', 'mean |mixed curvature|', 'Benchmark C backend curvature scale vs dephasing')
    ...
    for benchmark_id, row in summary_rows.items():
        md_lines.append(f"- {benchmark_id}: switch γ={row['recommended_switch_gamma']:.3f}, trusted={row['trusted_pair_count_at_switch']}, excluded={row['excluded_pair_count_at_switch']}, slope={row['fit_slope_at_switch']}, R²={row['fit_r2_at_switch']}, sign={row['alignment_sign']:+.0f}")
    ...
    md_lines.extend(
        [
            ...
            f"- Mean Bures distance between backends at common γ: {compare['mean_bures_distance_between_backends']}",
            ...
            f"- Lindblad fit at common γ: slope={compare['lindblad_fit_at_common_gamma']['slope']}, R²={compare['lindblad_fit_at_common_gamma']['r2']}, count={compare['lindblad_fit_at_common_gamma']['count']}",
            f"- Effective fit at common γ: slope={compare['effective_fit_at_common_gamma']['slope']}, R²={compare['effective_fit_at_common_gamma']['r2']}, count={compare['effective_fit_at_common_gamma']['count']}",
            '',
            f"Plot: `{slope_path}`",
            f"Plot: `{r2_path}`",
            f"Plot: `{curv_path}`",
