...
def _mixed_curvature_grid(rho_grid: list[list[np.ndarray]], ambiguous_map: list[list[bool]], du: float, dv: float) -> np.ndarray:
    mesh = len(rho_grid)
    curv = np.full((mesh - 1, mesh - 1), np.nan, dtype=float)
    area = du * dv
    for i in range(mesh - 1):
        for j in range(mesh - 1):
            if any([ambiguous_map[i][j], ambiguous_map[i + 1][j], ambiguous_map[i + 1][j + 1], ambiguous_map[i][j + 1]]):
                continue
            curv[i, j] = mixed_plaquette_curvature(rho_grid[i][j], rho_grid[i + 1][j], rho_grid[i + 1][j + 1], rho_grid[i][j + 1], area)
    return curv
...
def _loop_run(benchmark_id: str, center: tuple[float, float], side: float, orientation: str, loop_config: LoopConfig, dephasing: float, cache: DensityCache, step_fn) -> dict:
    benchmark = get_benchmark(benchmark_id)
    path = build_loop_path(center=center, side=side, orientation=orientation, shape=loop_config.shape, steps_per_segment=loop_config.steps_per_segment)
...
def _loop_levels_for_backend(benchmark_id: str, dephasing_values: tuple[float, ...], cache: DensityCache, step_fn, loop_config: LoopConfig) -> list[dict]:
    benchmark = get_benchmark(benchmark_id)
    levels: list[dict] = []
...
def _benchmark_payload(benchmark_id: str, output_root: Path, scan_config: ScanConfig, loop_config: LoopConfig, dephasing_values: tuple[float, ...], floor: float, cache: DensityCache, step_fn, backend_name: str) -> dict:
    benchmark = get_benchmark(benchmark_id)
    ...
    payload = {
        'phase': 'phase11_lindblad_continuous_time' if backend_name == 'lindblad' else 'phase10_effective_k_step_reference',
...
def phase11_payload(output_root: Path, phase11_config: Phase11Config | None = None, scan_config: ScanConfig | None = None, loop_config: LoopConfig | None = None, open_config: OpenSystemConfig | None = None, lindblad_config: LindbladConfig | None = None) -> dict:
    ...
    comparison = {
        ...
        'curvature_correlation_at_gamma': _corr(np.asarray(eff_scan['mixed_curvature'], dtype=float), np.asarray(lind_scan['mixed_curvature'], dtype=float)),
        'metric_correlation_at_gamma': _corr(np.asarray(eff_scan['bures_metric_trace'], dtype=float), np.asarray(lind_scan['bures_metric_trace'], dtype=float)),
    ...
    for benchmark_id, payload in benchmark_payloads.items():
        ...
        benchmark_summary[benchmark_id] = {
            ...
            'mean_abs_mixed_curvature_at_switch': _select_level(payload['scan_levels'], payload['recommended_switch_gamma'])['mean_abs_mixed_curvature'],
        }

    payload = {
        'phase': 'phase11_continuous_time_lindblad',
        'description': 'Continuous-time graph-local Lindblad-style extension plus broader mixed-state holonomy validation across the benchmark set.',
        'benchmark_summaries': benchmark_summary,
        'benchmark_payloads': benchmark_payloads,
        'benchmark_c_backend_comparison': comparison,
        'notes': [
            'The Lindblad lane is built from the same branch-local coherent Hamiltonian and graph-local jump structure as the earlier effective CPTP lane, but integrated as a continuous-time generator.',
            'Mixed-state fits remain patchwise; benchmark F is expected to exclude most or all loops because R4 switching dominates.',
            'Topology is still restricted to the auxiliary periodic/gapped sector and is not inferred from the mixed-state dynamic benchmarks here.',
        ],
    }
...
def phase11_report(output_root: Path, payload: dict) -> dict[str, Path]:
    ...
    if ref_payload is not None:
        ...
        _plot_lines(compare_path, x, [lind_curv, eff_curv], ['lindblad', 'effective'], 'dephasing γ', 'mean |mixed curvature|', 'Benchmark C mixed curvature scale by backend')
    else:
        _plot_lines(compare_path, x, [lind_curv], ['lindblad'], 'dephasing γ', 'mean |mixed curvature|', 'Benchmark C mixed curvature scale (Lindblad)')
    ...
    for benchmark_id, summary in payload['benchmark_summaries'].items():
        lines.append(
            f"- {benchmark_id}: switch γ={summary['recommended_switch_gamma']:.2f}, trusted pairs={summary['trusted_pair_count_at_switch']}, excluded pairs={summary['excluded_pair_count_at_switch']}, fit R²={summary['fit_r2_at_switch']}, mean |mixed curvature|={summary['mean_abs_mixed_curvature_at_switch']}"
        )
    lines.extend(
        [
            '',
            ...
            f"- mean Bures distance between effective and Lindblad branch densities: {compare['mean_bures_distance_between_backends']}",
            f"- metric correlation: {compare['metric_correlation_at_gamma']}",
            f"- curvature correlation: {compare['curvature_correlation_at_gamma']}",
            f"- effective fit at gamma: slope={compare['effective_global_fit_at_gamma']['slope']}, R²={compare['effective_global_fit_at_gamma']['r2']}, count={compare['effective_global_fit_at_gamma']['count']}",
            f"- Lindblad fit at gamma: slope={compare['lindblad_global_fit_at_gamma']['slope']}, R²={compare['lindblad_global_fit_at_gamma']['r2']}, count={compare['lindblad_global_fit_at_gamma']['count']}",
            '',
            f"Plot: `{scatter_path}`",
            f"Plot: `{compare_path}`",
            '',
            ...
            '- Benchmark F remains dominated by R4 exclusions, which is the expected behavior for an explicit branch-switching benchmark.',
            '',
        ]
    )
    md_path.write_text('\n'.join(lines))
    return {'phase11_report': md_path, 'benchmark_c_scatter': scatter_path, 'benchmark_c_backend_curve': compare_path}
