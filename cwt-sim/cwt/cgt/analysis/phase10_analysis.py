...
def _scan_level(benchmark_id: str, scan_config: ScanConfig, open_config: OpenSystemConfig, cache: FixedPointCache, dephasing: float) -> dict:
...
def _pair_row(ccw: dict, cw: dict) -> dict:
    ...
    return {
        ...
        'mixed_holonomy_abs_mean': float(0.5 * (abs(ccw['mixed_holonomy_phase']) + abs(cw['mixed_holonomy_phase']))),
...
def phase10_payload(
    ...
    open_cfg = open_config or OpenSystemConfig(scan_mesh=p10.scan_mesh, dephasing_values=p10.dephasing_values, coherence_switch_floor=p10.coherence_switch_floor)
    cache = FixedPointCache(open_cfg)

    benchmark_dir = output_root / benchmark.slug
    benchmark_dir.mkdir(parents=True, exist_ok=True)

    scan_levels = [_scan_level(benchmark.benchmark_id, scan_cfg, open_cfg, cache, gamma) for gamma in p10.dephasing_values]
    ...
    selected_level = next(level for level in loop_levels if abs(float(level['dephasing']) - switch_gamma) < 1e-9)
    ...
    payload = {
        ...
        'slug': benchmark.slug,
        'description': 'Graph-local open-system lane plus genuine mixed-state holonomy / curvature diagnostics from an effective K-step noisy branch map.',
        'open_system_config': {
            'dt': open_cfg.dt,
        ...
        'notes': [
            'The noisy lane is now generated by a graph-local CPTP map built from the branch kernel and branch phase field, not by direct convex mixing with the target density.',
            'The mixed-state phase object is an Uhlmann-like holonomy computed from polar link unitaries of neighboring density amplitudes.',
            'Fits remain patchwise; the theory should not claim a single global susceptibility over the full control domain.',
        ],
    }
...
def _save_scatter_plot(path: Path, xs: np.ndarray, ys: np.ndarray, xlabel: str, ylabel: str, title: str, fit: dict[str, float | int | None]) -> None:
...
def phase10_report(output_root: Path, payload: dict) -> dict[str, Path]:
    ...
    level = next(level for level in payload['loop_levels'] if abs(float(level['dephasing']) - switch_gamma) < 1e-9)
    ...
    for center_key, fit in payload['patchwise_fit_at_switch_gamma'].items():
        patch_lines.append(f"- center {center_key}: slope={fit['slope']}, R²={fit['r2']}, count={fit['count']}")
    md_text = f"""# CWT-CGT Phase 10 Report
...
Recommended mixed-state switch: **γ ≈ {switch_gamma:.2f}**.

Global fit at the switch point:
- slope = {payload['global_fit_at_switch_gamma']['slope']}
- R² = {payload['global_fit_at_switch_gamma']['r2']}
- trusted pair count = {payload['global_fit_at_switch_gamma']['count']}

Patchwise fits at the switch point:
{chr(10).join(patch_lines)}

Coherence trend:
- γ grid = {payload['dephasing_values']}
- mean |mixed curvature| by γ = {[format(level['mean_abs_mixed_curvature'], '.3e') for level in payload['scan_levels']]}
