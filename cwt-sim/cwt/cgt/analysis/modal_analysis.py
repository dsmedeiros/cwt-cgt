    ...
    for i in range(rows - 1):
        for j in range(cols - 1):
            ...
            mixing_plaquette[i, j] = float(
                np.mean(
                    [off_diagonal_mixing_score(a, b) for a, b in zip(local_frames[:-1], local_frames[1:])]
                )
            )
...
def modal_loop_payload(
    ...
    loop_payload = run_benchmark_loops(
        benchmark_id, output_root=output_root, config=config, protocol_name=protocol_name
    )
    run_records = []
    for pair in loop_payload["pair_results"]:
        for run in [pair["ccw"], pair["cw"]]:
            ...
            frames = [
                biorthogonal_modal_frame(auxiliary_operator_from_state(state))
    ...
    payload = {
        ...
        "summary": {
            ...
            "max_biorthogonality_error": (
                None if not trusted else float(max(run["max_biorthogonality_error"] for run in trusted))
            ),
        },
        "notes": [
            "Modal Wilson phases are computed for the dominant mode of the auxiliary operator"
            " along trusted branch-following loops.",
            "Trusted runs inherit the passive R1 filter from the loop layer and exclude R4"
            " branch-switching paths.",
        ],
    }
    out_dir = output_root / benchmark.slug
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / (filename or f"{benchmark.benchmark_id}_modal_loops.json")).write_text(
        json.dumps(payload, indent=2)
    )
    return payload


def benchmark_modal_payload(
    benchmark_id: str, output_root: Path, scan_config: ScanConfig | None = None
) -> dict:
...
def patchwise_modal_explanation(output_root: Path) -> dict:
    ...
    for protocol in protocol_suite("benchmark_c"):
        loops = run_benchmark_loops(
            "benchmark_c", output_root=output_root, config=protocol.config, protocol_name=protocol.name
        )
        ...
        center_summaries.append(
            {
                ...
                "relative_error": (
                    None
                    if abs(flux_slope) <= 1e-12
                    else float(abs(predicted_flux_slope - flux_slope) / abs(flux_slope))
                ),
            }
        )

    observed = np.asarray(
        [item["mean_response_vs_signed_flux_slope"] for item in center_summaries], dtype=float
    )
    predicted = np.asarray(
        [item["predicted_flux_slope_from_modal_factors"] for item in center_summaries], dtype=float
    )
    modal_only = np.asarray(
        [item["mean_response_vs_modal_phase_slope"] for item in center_summaries], dtype=float
    )
    payload = {
        ...
        "notes": [
            "Patchwise slope variation is decomposed into response-per-modal-phase and"
            " modal-phase-per-signed-flux factors.",
            "In the current benchmark C implementation, most center-to-center variation lives"
            " in the response-per-modal-phase factor.",
        ],
    }
    out_dir = output_root / benchmark.slug
