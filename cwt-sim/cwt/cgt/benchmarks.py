...
def _branch_f(u: float, v: float, sign: float) -> BranchState:
    ...
    tilt = np.exp(np.array([1.00 * tilt_strength, 0.30 * tilt_strength, -0.30 * tilt_strength, -1.00 * tilt_strength], dtype=float))
    p = normalize_probabilities(base * tilt)
    ...
    return BranchState(
        ...
        theta=theta,
        kernel=kernel,
        extras={'u': float(u), 'v': float(v), 'branch_sign': float(sign), 'center_closeness': float(center_closeness)},
    )
...
BENCHMARKS: dict[str, BenchmarkDefinition] = {
    ...
    'benchmark_f': BenchmarkDefinition(
        ...
        expected_behavior='Wide R4 bistable band, forward/backward branch disagreement, and loop exclusions by construction.',
        expected_regime='R4',
        control_names=('u', 'v'),
