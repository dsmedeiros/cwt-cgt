...
def apply_cptp_relaxation(rho: np.ndarray, target: np.ndarray, relaxation: float, dephasing: float, depolarizing: float) -> np.ndarray:
...
def noisy_scan_level(benchmark_id: str, dephasing: float, config: NoisyConfig, scan_config: ScanConfig | None = None) -> dict:
    benchmark = get_benchmark(benchmark_id)
...
def noisy_benchmark_payload(
    ...
    scan_levels = [noisy_scan_level(benchmark_id, dephasing=gamma, config=noisy_cfg, scan_config=scan_cfg) for gamma in noisy_cfg.dephasing_values]
                ...
                pairs.append(
                    {
                        ...
                        'mean_coherence_ratio': float(0.5 * (ccw['avg_coherence_ratio'] + cw['avg_coherence_ratio'])),
                    }
                )
    ...
    payload = {
        ...
        'notes': [
            ...
            'Scan-side geometry currently reports Bures-metric traces and coherence ratios; a full Uhlmann-curvature implementation remains future work.',
            'Loop-side results are reported both globally and patchwise by center. In the ring benchmark, the patchwise orientation-gap law is stronger than the global response-vs-flux fit.',
        ],
    }
