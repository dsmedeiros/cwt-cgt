...
def protocol_suite(benchmark_id: str) -> list[RobustnessProtocol]:
    ...
    return [
        RobustnessProtocol(
            ...
            config=LoopConfig(
                shape="square", centers=benchmark.default_loop_centers, side_lengths=default_sides
            ),
        ...
        RobustnessProtocol(
            ...
            config=LoopConfig(
                shape="circle", centers=benchmark.default_loop_centers, side_lengths=default_sides
            ),
...
def _evaluate_payload(benchmark_id: str, payload: dict) -> dict[str, str | bool | int | float | None]:
    ...
    if benchmark_id == "benchmark_c" and protocol_name.startswith("offcenter_"):
        ...
        reason = (
            "positive loop law persists with center-dependent local susceptibility"
            if passed
            else "off-center protocol does not yet show a stable centerwise positive law"
        )
        ...
        passed = (
            ...
            and abs(float(slope)) > 1e-4
            and r2 is not None
            and float(r2) > 0.90
        )
        reason = (
            "positive loop law persists under this protocol"
            if passed
            else "positive loop law weak or not yet stable under this protocol"
        )
    else:
        passed = (
            trusted_pairs >= 2
            and sign_flip <= 0.25
            and (slope is None or abs(float(slope)) < 1e-3)
            and orientation_gap_max < 1e-6
        )
        reason = (
            "null-like benchmark remains null under this protocol"
            if passed
            else "null-like benchmark shows suspicious signed-loop structure"
        )
...
def run_robustness_suite(benchmark_id: str, output_root: Path) -> dict:
    ...
    payload = {
        ...
        "notes": [
            "Robustness protocols are held-out loop families and off-center center sets evaluated on"
            " top of the passive benchmark block.",
            "These protocols do not alter the branch geometry itself; they only change the loop family"
            " used to probe the same state manifold.",
        ],
    }
