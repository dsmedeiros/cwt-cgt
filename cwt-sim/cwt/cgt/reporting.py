...
def evaluate_benchmark(scan: dict, loops: dict) -> dict[str, str | bool]:
    ...
    if benchmark == "benchmark_c":
        ...
        reason = (
            "null benchmark remains null-like" if passed else "null benchmark shows suspicious loop structure"
        )
...
def write_benchmark_report(
    scan: dict, loops: dict, plots: dict[str, str], output_path: Path
) -> dict[str, str | bool]:
    ...
    lines.append(
        f"**Observed verdict:** {'PASS' if evaluation['passed'] else 'REVIEW'} — {evaluation['reason']}"
    )
    lines.append("")
    ...
    lines.append(
        f"- Fit slope vs signed flux:"
        f" {_fmt(loop_summary.get('fit_response_vs_signed_flux', {}).get('slope'))}"
    )
    lines.append(
        f"- Fit R² vs signed flux: {_fmt(loop_summary.get('fit_response_vs_signed_flux', {}).get('r2'))}"
    )
...
def write_aggregate_report(
    scans: Iterable[dict], loops_list: Iterable[dict], evaluations: dict[str, dict], output_path: Path
) -> None:
    scans = list(scans)
    ...
    lines.append("")
    lines.append(
        "This report is generated from the current benchmark JSON artifacts and the plot/report"
        " automation layer."
    )
    lines.append("")
    for scan in scans:
        ...
        lines.append(
            f"- Regimes: {', '.join(f'{k}:{v}' for k, v in sorted(scan.get('regime_summary', {}).items()))}"
        )
        ...
        lines.append(
            f"- Flux-fit slope / R²:"
            f" {_fmt(summary.get('fit_response_vs_signed_flux', {}).get('slope'))}"
            f" / {_fmt(summary.get('fit_response_vs_signed_flux', {}).get('r2'))}"
        )
        lines.append("")
