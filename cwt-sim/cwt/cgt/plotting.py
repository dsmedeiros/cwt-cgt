...
def _save_category_grid(labels: list[list[str]], scan: dict, title: str, outpath: Path, value_order: list[str]) -> None:
    ...
    fig, ax = plt.subplots(figsize=(6.2, 5.0))
    image = ax.imshow(arr, origin='lower', aspect='auto', extent=_extent(scan), cmap='tab10', vmin=0, vmax=max(len(value_order) - 1, 1))
    ax.set_title(title)
    ax.set_xlabel(scan['control_names'][1])
    ax.set_ylabel(scan['control_names'][0])
...
def _scatter_with_fit(x: np.ndarray, y: np.ndarray, trusted: np.ndarray, xlabel: str, ylabel: str, title: str, slope: float | None, outpath: Path) -> None:
    fig, ax = plt.subplots(figsize=(6.2, 5.0))
...
def plot_regime_map(scan: dict, outpath: Path) -> None:
    _save_category_grid(scan['regime_map'], scan, 'Regime map', outpath, ['R1', 'R2', 'R3', 'R4', 'unresolved'])
